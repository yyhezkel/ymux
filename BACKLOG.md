...
- [ ] 2026-09-01 | from-plan:brief        | **P2** - Queue panel "refresh the picture 🤖" button: on-demand LLM pass over the live sessions' transcripts producing the rich narrative column from Yossi's reference table (what the self-reported briefs approximate for free). Explicitly deferred from BRIEF v1 — button-triggered only, never automatic per switch.

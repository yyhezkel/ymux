...
- [ ] 2026-09-02 | manual                 | **P2** - BRIEF ⇄ sessions-overview unification, half 2: the overview pays a `claude -p` call per row (`app/src-tauri/src/sessions_overview.rs`) even when the pane has a fresh self-reported brief (free, in `AppState.briefs`) — prefer the brief, LLM only for sessions with no brief. And the reverse: BRIEF's deferred "refresh the picture 🤖" button should reuse sessions_overview's summarize pipeline rather than new plumbing. This SUPERSEDES the 2026-09-01 from-plan:brief entry about that button (removed here).
- [ ] 2026-09-01 | from-plan:brief        | **P3** - seed the skills registry with a `ymux-brief` skill (SKILL.md teaching agents the `[ymux-brief]` block). Installer + UI already exist (`skills.rs`, YmuxToolsTab) but the registry ships empty — needs bundling as a Tauri resource + startup seeding. v1 ships a copy-paste CLAUDE.md snippet in the Queue empty-state instead.
